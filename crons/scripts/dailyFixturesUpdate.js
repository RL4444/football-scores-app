...
const job = async () => {
    const todaysJobs = JSON.parse(
        fs.readFileSync(path.join(__dirname, "../jobs.json"))
    );
    ...
    todaysJobs.jobs.forEach(async (job) => {
        const matchHappeningNow =
            moment().isAfter(job.from) && moment().isBefore(job.to);
        ...
        console.log(
            job.games.map((gme) => gme).join(" "),
            "happening -- scraping scores"
        );
        const { data, error } = await getFixturesAndResults(
            job.url,
            true,
            keys[job.competition].league,
            false
        );
        if (error) {
        ...
        if (result) {
            console.log({ result });
            console.log(
                `updated scores for ${job.games.map((gme) => gme).join(" ")}`
            );
        }
    });
};
